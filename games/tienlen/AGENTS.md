...
log that a later settlement step will consume; legal-move enumeration;
and the Player interface. Later steps add the settlement ledger, the AI
players, and the Standard variant.
...
| Change settlement inputs | `events.go` | The event log feeds the later ledger; keep it append-only and complete |
...
- Events are the settlement record: append-only inside the engine,
  ...
  nothing. The ledger (a later step) folds them into transfers that net
  to zero; the state machine never computes payments.
