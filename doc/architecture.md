...
## Dependency Policy
`cardcore` maintains zero external runtime dependencies. It relies exclusively on the Go standard library, using `math/rand/v2` for shuffling and `sort` for hand management. This keeps the binary small and ensures reproducible builds. Dev tool versions (e.g., golangci-lint) are pinned in `go.mod` via the `tool` directive for reproducible builds across contributors and CI. Go 1.25.12+ is required for `sync.WaitGroup.Go`, used in the Hearts PIMC AI player's parallel sample loop.
